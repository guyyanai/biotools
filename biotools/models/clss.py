...
class CLSS(pl.LightningModule):
    def __init__(self, model_name: str, hidden_dim: int, learning_rate: float = 1e-3, T_max: int = 10, dropout_rate: bool = 0.2, should_use_scheduler: bool = False, extended_esm3_proj: bool = False, esm3_attention: bool = False, esm3_attention_heads: bool = 8, random_sequence_stretches: bool = False, random_stretch_min_size: int = 30):
        ...
        self.esm2_projection_head = nn.Sequential(
            nn.ReLU(),
            # nn.Dropout(drop_rate), drop_rate: float = 0.2
            nn.Linear(self.esm2_model.config.hidden_size, hidden_dim)
        )
        
        if extended_esm3_proj:
            self.esm3_projection_head = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Dropout(dropout_rate),
                nn.Linear(480, hidden_dim),
            )
        elif esm3_attention:
            self.esm3_projection_head = nn.Sequential(
                nn.MultiheadAttention(embed_dim=1536, num_heads=esm3_attention_heads),
        ...
        self.should_use_scheduler = should_use_scheduler
        self.T_max = T_max
        self.random_sequence_stretches = random_sequence_stretches
        self.random_stretch_min_size = random_stretch_min_size
        self.model_name = model_name

    def forward(self, batched_input_ids, batched_attention_mask, batched_structure_embeddings):
        ...
        for index in range(batched_input_ids.shape[0]):
            ...
            if self.random_sequence_stretches:
                if sequence_length < self.random_stretch_min_size:
                    ...
                    start_index = torch.randint(0, sequence_length - self.random_stretch_min_size + 1, (1,)).item()
                    max_length = sequence_length - start_index
                    substring_length = torch.randint(self.random_stretch_min_size, max_length + 1, (1,)).item()
                
                stretch_lengths[index] = substring_length
                input_ids = input_ids[start_index:start_index + substring_length]
                attention_mask = attention_mask[start_index:start_index + substring_length]
                
            sequence_output = self.esm2_model(input_ids=input_ids[:sequence_length].unsqueeze(0), attention_mask=attention_mask[:sequence_length].unsqueeze(0))
    ...
    def contrastive_loss(self, projections1, projections2, temperature: float = 0.5):
        ...
        similarities = torch.mm(projections1, projections2.T) / temperature
    ...
    def configure_optimizers(self):
        ...
        if self.should_use_scheduler:
            ...
            return {
                ...
                'lr_scheduler': {
                    'scheduler': scheduler,
                    'interval': 'epoch',  # Update the learning rate at the end of each epoch
