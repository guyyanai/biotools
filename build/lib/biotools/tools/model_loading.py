...
def load_weights_from_checkpoint(checkpoint_path: str, weights_key_prefix: str):
    ...
    for key in checkpoint["state_dict"]:
        if key.startswith(
            weights_key_prefix + "."
        ):  # Adjust this based on your model's naming convention
            weights[".".join(key.split(".")[1:])] = checkpoint["state_dict"][key]

    if weights is {}:
        raise ValueError(
            f"Weights not found in the checkpoint!\nKey Prefix: {weights_key_prefix}"
        )

    return weights


def load_esm2_from_checkpoint(
    esm2_config: str, checkpoint_path: str, weights_key_prefix: str
):
...
def load_clss_v1_1(clss_checkpoint: str, device=None):
    # TODO: change this to normal loading
    if device is None:
        device = torch.device("cuda")
...
def load_clss_v1_1_state_dict(clss_checkpoint: str, device=None):
    # TODO: change this to normal loading
    if device is None:
        device = torch.device("cuda")

    state_dict = torch.load(clss_checkpoint)
    model = CLSSv1_1("facebook/esm2_t12_35M_UR50D", 32, should_load_esm3=False)
...
def load_clss_v2(clss_checkpoint: str, device=None):
    # TODO: change this to normal loading
    if device is None:
        device = torch.device("cuda")

    model = CLSSv2.load_from_checkpoint(clss_checkpoint, strict=False).to(device)
