...
class CLSSv1_2(pl.LightningModule):
    def __init__(
        self,
        esm2_checkpoint: str,
        hidden_dim: int,
        learning_rate: float = 1e-3,
        init_temperature=torch.tensor(0.5, dtype=torch.float32),
    ...
    def embed_sequences(self, sequences: List[str]) -> torch.Tensor:
        ...
        for sequence in sequences:
            ...
            embedding_projection = self.sequence_projection_head(embedding)
    ...
    def forward(
        ...
        for index in range(batched_input_ids.shape[0]):
            ...
            sequence_projection = self.sequence_projection_head(sequence_output)
            mean_sequence_projection = sequence_projection.mean(dim=0)
            sequence_outputs.append(mean_sequence_projection)
        ...
        sequence_projections = torch.stack(sequence_outputs)
