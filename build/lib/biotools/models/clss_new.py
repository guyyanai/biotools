...
class NewCLSS(pl.LightningModule):
    def __init__(self, esm2_checkpoint: str, hidden_dim: int, learning_rate: float = 1e-3, temperature: float = 0.5, random_sequence_stretches: bool = False, random_stretch_min_size: int = 30, should_load_esm3: bool = True):
        super(NewCLSS, self).__init__()
    ...
    def sample_random_stretch(self, input_ids: torch.Tensor, attention_mask: torch.Tensor):
        ...
        if sequence_length < self.random_stretch_min_size:
            ...
            start_index = int(torch.randint(0, sequence_length - self.random_stretch_min_size + 1, (1,)).item())
            
            max_length = sequence_length - start_index
            substring_length = int(torch.randint(self.random_stretch_min_size, max_length + 1, (1,)).item())
    ...
    def gather_projections(self, projections: torch.Tensor) -> torch.Tensor:
        gathered = self.all_gather(projections)

        # Reshape tensors if necessary
        if isinstance(gathered, list):
            gathered = torch.cat(gathered, dim=0)
        else:
            gathered = gathered.view(-1, projections1.size(-1)) # type: ignore
    ...
    def contrastive_loss(self, projections1: torch.Tensor, projections2: torch.Tensor):
        ...
        self.log('actual_loss_total_samples', projections1.shape[0], prog_bar=True)
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log(f'train_batch_size_per_gpu_{self.trainer.global_rank}', structure_embeddings.shape[0], prog_bar=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Log the number of samples in this GPU's batch
        self.log(f'validation_batch_size_per_gpu_{self.trainer.global_rank}', structure_embeddings.shape[0], prog_bar=True)
