...
from transformers import EsmModel, EsmTokenizer, DataCollatorForLanguageModeling
...
class CLSSv1(pl.LightningModule):
    def __init__(self, esm2_checkpoint: str, hidden_dim: int, learning_rate: float = 1e-3, temperature: float = 0.5, mask_probability: float = 0.15, random_sequence_stretches: bool = False, random_stretch_min_size: int = 30, should_load_esm3: bool = True):
        ...
        self.sequence_collator = self.create_sequence_collator()
        ...
        self.structure_projection_head = nn.Sequential(
            nn.Linear(1536, hidden_dim)
        )
        

    def create_sequence_collator(self) -> DataCollatorForLanguageModeling:
        # Create the data collator with 15% masking probability
        return DataCollatorForLanguageModeling(
            tokenizer=self.sequence_tokenizer,
            mlm=True,
            mlm_probability=self.hparams['mask_probability']
        )
    ...
    def embed_sequences(self, sequences: List[str]) -> torch.Tensor:
        ...
        for sequence in sequences:
            tokenized_sequence = self.sequence_tokenizer(sequence, return_tensors="pt").to(self.device)
    ...
    def embed_structures(self, structures: List[torch.Tensor]) -> torch.Tensor:
        if self.hparams['should_load_esm3']:
            raise Exception('should_load_esm3 flag is off, please turn it on to embed structures using CLSS')
    ...
    def sample_random_stretch(self, input_ids: torch.Tensor, attention_mask: torch.Tensor):
        sequence_length = int(attention_mask.count_nonzero().item())
        
        if sequence_length < self.hparams['random_stretch_min_size']:
            start_index = 0
            substring_length = sequence_length
        else:
            start_index = int(torch.randint(0, sequence_length - self.hparams['random_stretch_min_size'] + 1, (1,)).item())
            
            max_length = sequence_length - start_index
            substring_length = int(torch.randint(self.hparams['random_stretch_min_size'], max_length + 1, (1,)).item())
        
        stretch_input_ids = input_ids[start_index:start_index + substring_length]
        stretch_attention_mask = attention_mask[start_index:start_index + substring_length]

        return stretch_input_ids, stretch_attention_mask, substring_length

    def forward(self, batched_input_ids: torch.Tensor, batched_attention_mask: torch.Tensor, batched_structure_embeddings: torch.Tensor):
        # Get the outputs from the ESM model
        sequence_outputs = []
        
        # Mask sequence tokens    
        batched_input_ids = self.sequence_collator(batched_input_ids)
        ...
        for index in range(batched_input_ids.shape[0]):
            ...
            if self.hparams['random_sequence_stretches']:
                input_ids, attention_mask, stretch_length = self.sample_random_stretch(input_ids, attention_mask)
                stretch_lengths[index] = stretch_length
            
            sequence_output = self.sequence_encoder(input_ids=input_ids[:sequence_length].unsqueeze(0), attention_mask=attention_mask[:sequence_length].unsqueeze(0))
        ...
        structure_projections = self.structure_projection_head(batched_structure_embeddings)
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log(f'train_batch_size_per_gpu_{self.trainer.global_rank}', structure_embeddings.shape[0])
        
        # Forward pass for both pairs
        sequence_projections, structure_projections = self(input_ids, attention_mask, structure_embeddings)
        ...
        self.log('gradient_norm', gradient_norm.detach().cpu(), prog_bar=True, logger=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Log the number of samples in this GPU's batch
        self.log(f'validation_batch_size_per_gpu_{self.trainer.global_rank}', structure_embeddings.shape[0])
        ...
        self.log('val_loss', val_loss.detach(), prog_bar=True, logger=True, sync_dist=True)
